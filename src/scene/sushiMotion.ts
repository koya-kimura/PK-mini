...
export class SushiMotion {
    ...
    update(sequenceValue: number, currentBeat: number): void {
        ...
        for (let i = 0; i < this.SUSHI_COUNT; i++) {
            ...
            const positionY = p.height * 0.5; // 画面中央 (Y) に固定

            // 寿司のサイズを決定
            const sushiScale = drawingRange / (this.SUSHI_COUNT * 1.2);

            // ネタをランダムに決定 (ビートで固定される)
            const netaType = p.random(Object.values(this.NETA_TYPES));

            // 舎利とネタを描画
            this.drawSumeshi(positionX, positionY, sushiScale);
            this.drawSushiNeta(netaType, positionX, positionY, sushiScale);
        }
        p.pop();
    }
     ...
     * 舎利（ご飯部分）を描画します。
     */
    private drawSumeshi(x: number, y: number, s: number): void {
        const p = this.p;
        ...
        p.fill(255); // 白
        // 舎利の形状 (ネタの下に位置)
        p.rect(0, s * 0.4, s * 0.7, s * 0.4, s*0.02, s*0.02);
        p.pop();
    }
     ...
     */
    private drawSushiNeta(neta: string, x: number, y: number, s: number): void {
        switch (neta) {
            case this.NETA_TYPES.MAGURO:
                this.drawNetaShape(this.NETA_TYPES.MAGURO, x, y, s);
                break;
            case this.NETA_TYPES.SALMON:
                this.drawNetaShape(this.NETA_TYPES.SALMON, x, y, s);
                break;
            case this.NETA_TYPES.TAMAGO:
                this.drawNetaShape(this.NETA_TYPES.TAMAGO, x, y, s);
                break;
            default:
                this.drawNetaShape(this.NETA_TYPES.MAGURO, x, y, s);
                break;
     ...
     */
    private drawNetaShape(netaType: string, x: number, y: number, s: number): void {
        const p = this.p;
        ...
        p.rect(0, netaYOffset, netaWidth, netaHeight, p.min(netaHeight, netaWidth) * 0.05);

        // 2. マグロとサーモンの縞模様を描画 (ここで型ガード/絞り込みが行われる)
        if (netaType === this.NETA_TYPES.MAGURO || netaType === this.NETA_TYPES.SALMON) {
            // エラーを回避するため、安全な型アサーションを使用してstripeプロパティを取得
            ...
            p.fill(stripeColor[0], stripeColor[1], stripeColor[2]);
            ...
            for (let i = 0; i < this.STRIPE_COUNT; i++) {
                ...
                // 縞模様は幅全体に広がる線として描画
                // 縞を細い矩形として描画しています
                p.rect(stripeX, netaYOffset, stripeWidth * 0.2, netaHeight);
            }
        }
        ...
        if (netaType === this.NETA_TYPES.TAMAGO) {
            p.fill(0); // 黒
            p.rect(0, netaYOffset, s * 0.2, netaHeight);
        }
