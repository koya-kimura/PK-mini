...
export class WaveMotion {
     ...
     */
    private drawSeigaihaUnit(centerX: number, centerY: number, radiusW: number, radiusH: number = radiusW): void {
        const p = this.p;
        ...
        for (let i = 0; i < this.ARC_SEGMENTS; i++) {
            ...
            p.arc(0, 0, currentRadiusW * 2, currentRadiusH * 2, p.PI, p.TWO_PI);
        }
        p.pop();
    }
    ...
    update(sequenceValue: number, currentBeat: number): void {
        ...
        // 基本サイズと間隔の計算
        const baseRadius = p.min(p.width, p.height) * this.BASE_RADIUS_RATIO; // ユニットの基準半径
        const stepX = baseRadius * 2;             // X方向の間隔（直径）
        const stepY = baseRadius * this.Y_STEP_RATIO; // Y方向の間隔

        // 描画する行数と列数を計算（画面を覆うのに十分な数）
        // *** 修正箇所1: BASE_Y_START_RATIO に修正 ***
        const rows = p.floor((p.height * (1 - this.BASE_Y_START_RATIO) / stepY) * 1.5) + 2;
        const cols = p.floor(p.width / stepX) + 2;
        ...
        for (let j = 0; j < rows; j++) {
            ...
            for (let i = -1; i < cols; i++) {
                ...
                // *** 修正箇所2: BASE_Y_START_RATIO に修正 ***
                let centerY = p.height * this.BASE_Y_START_RATIO + j * stepY + waveOffset;
                ...
                // 青海波の基本ユニットを描画
                this.drawSeigaihaUnit(centerX, centerY, radiusW, radiusH);
            }
        }
